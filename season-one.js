const chaptersData = [
            // Prelude
            {
                title: "Prelude: R<sup>3</sup> Model",
                ...
                content: `
                    <div class="prelude-container">
                        ...
                        <div class="prelude-section active" id="narrative-section">
                            ...
                            <p>That's what I now call the R<sup>3</sup> Model™. It's not academic. It's lived. And it became the foundation for how I began healing, leading, and helping others grow - not from perfection, but from presence.</p>
                        </div>
                ...
                contextualData: `
                    <div class="content-section">
                        <h3><i class="fas fa-lightbulb"></i> Conceptual Application of the R<sup>3</sup> Model</h3>
                        <p>Growth doesn’t begin at the edge. It begins at the center. Not in performance. In presence.</p>
                        <p>The R<sup>3</sup> Model - Reflection, Resistance, Revelation - holds the architecture of growth that starts within.</p>
                ...
                resources: `
                    ...
                    <div class="content-section">
                        ...
                        <p>Practical exercises to apply the R<sup>3</sup> Model:</p>
                        <ul class="prompt-list">
...
const renderChapters = () => {
  ...
  chaptersData.forEach((chapter, index) => {
    ...
    header.innerHTML = `
      <h2>${chapter.title}</h2>
      ${chapter.theme ? `<p class="chapter-theme">${chapter.theme}</p>` : ''}
    `;

    const narrative = document.createElement('div');
    narrative.className = 'chapter-narrative';
    narrative.innerHTML = chapter.content;

    const contextual = document.createElement('div');
    contextual.className = 'chapter-context';
    contextual.innerHTML = chapter.contextualData || '';

    const resources = document.createElement('div');
    resources.className = 'chapter-resources';
    resources.innerHTML = chapter.resources || '';

    section.appendChild(header);
    section.appendChild(narrative);
    section.appendChild(contextual);
    section.appendChild(resources);
    chaptersContainer.appendChild(section);

    const tocItem = document.createElement('li');
    tocItem.innerHTML = `<a href="#chapter-${index}">${chapter.title}</a>`;
...
const configureToc = () => {
  ...
  tocDetails.open = isDesktop;
};

const setupPreludeTabs = () => {
  const tabContainers = document.querySelectorAll('.prelude-tabs');
  tabContainers.forEach((container) => {
    const tabs = container.querySelectorAll('.prelude-tab');
    const sections = container.parentElement.querySelectorAll('.prelude-section');
    ...
    tabs.forEach((tab) => {
      tab.addEventListener('click', () => {
        ...
        const targetSection = container.parentElement.querySelector(`#${target}-section`);
        if (targetSection) {
