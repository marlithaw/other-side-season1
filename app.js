    ...
    if (contactForm) {
        ...
        const validateField = (input) => {
            ...
            if (input.type === 'email' && input.value.trim()) {
                ...
                if (!isValid) {
                    showError(input.id, 'Enter a valid email address.');
                    return false;
                }
            }
        ...
        contactForm.addEventListener('submit', (event) => {
            ...
            if (!isValid) {
                ...
                if (formNote) {
                    formNote.textContent = 'Please correct the highlighted fields before submitting.';
                }
            } else if (formNote) {
...
const chaptersData = [
        ...
        const formatR3 = (value) => (value ? value.replace(/R³/g, 'R<sup>3</sup>') : value);
        ...
        const buildChapterList = () => {
            ...
            chaptersData.forEach((chapter, index) => {
                ...
                button.type = 'button';
                button.innerHTML = formatR3(chapter.title);
        ...
        const updateChapterListState = (index) => {
            if (!chapterList) return;
            chapterList.querySelectorAll('button').forEach((button, buttonIndex) => {
        ...
        const renderChapter = (index) => {
            ...
            const formattedTitle = formatR3(chapter.title);
            const formattedContent = formatR3(chapter.content);
            const formattedContext = formatR3(chapter.contextualData);
            const formattedResources = formatR3(chapter.resources);

            if (chapterNavTitle) {
                chapterNavTitle.innerHTML = formattedTitle;
            }

            const narrativeSection = `
                <section id="chapter-narrative" class="content-section">
                    <h3><i class="fas fa-bookmark"></i> Narrative Anchor</h3>
                    ${formattedContent}
                </section>
            `;

            const contextSection = chapter.contextualData
                ? `<section id="chapter-context">${formattedContext}</section>`
                : '<section id="chapter-context"><!-- TODO: Add contextual content when available. --></section>';

            const resourcesSection = chapter.resources
                ? `<section id="chapter-resources">${formattedResources}</section>`
                : '<section id="chapter-resources"><!-- TODO: Add resources when available. --></section>';
            ...
            if (index === 0) {
                ...
                tabs.forEach((tab) => {
                    tab.addEventListener('click', () => {
                        ...
                        document.querySelectorAll('.prelude-section').forEach((section) => {
